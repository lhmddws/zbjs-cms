...
function init() {
  ...
  for (let i = 0; i < flipObjs.value.length; i++) {
    if (flipObjs.value[i] && flipObjs.value[i].value && typeof flipObjs.value[i].value.setFront === 'function') {
...
function run() {
  timer.value = setInterval(() => {
    ...
    for (let i = 0; i < flipObjs.value.length; i++) {
      ...
      if (flipObjs.value[i] && flipObjs.value[i].value && typeof flipObjs.value[i].value.flipDown === 'function') {
...
onMounted(() => {
  flipObjs.value = [flipperHour, flipperMinute, flipperSecond].filter(f => f != null)
  init()
  run()
})
</script>
